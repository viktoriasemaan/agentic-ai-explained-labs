# 🛡️ Lesson 2: How do you stop an AI agent from overspending or taking unsafe actions?

*Part of the **Agentic AI Explained** series · Lesson 2 of 10 · Last updated: August 27, 2026*
...
<p align="center">
  <img src="./assets/ai-guardrails-explained.gif" alt="Animated diagram: agent requests pass through a guardrail layer that runs PII, unsafe-content, and injection checks and returns allow, ask, or deny before any tool executes" width="480">
</p>
...
- 💸 **Cost, and parallelism makes it sharper.** An agent that dispatches four sub-agents is running four bills at once, and a retry loop in any one of them keeps calling a model until somebody notices.
- ⚠️ **Action risk.** A single shell command is usually harmless. A long sequence of shell commands in one session is a different situation, and evaluating each command on its own misses that.
...
## 🚧 Policies, guardrails, and why they are not prompts

A **guardrail** is a rule that is enforced *outside* the model. That distinction is the whole point. If a rule lives in the system prompt — "never delete files," "stop at $10" — then a prompt injection, a confused tool response, or a long context can talk the agent out of it. A guardrail evaluated in the runtime cannot be argued with, because the rule does not live in the prompt.

The policy layer is where guardrails run. The check happens **while the agent is operating, before the action executes.** Policies can also evaluate responses *after* a call returns.
...
Omnigent ships **more than 20 runtime policies** covering cost budgets, PII scanning, dangerous command blocking, and intent gating. Custom policies add organization-specific logic. They are evaluated server side in the Omnigent runtime on every tool call, for any harness in the session.
...
## 🗂️ Which policies belong to security and which to operations
...
| ⚙️ Behavior | Tool call limits, model allowlists, restrictions on external endpoints |
